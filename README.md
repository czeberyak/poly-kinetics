...
> * **Точность (Random Forest):** Модель показала высокую предсказательную способность с коэффициентом детерминации $R^2$ = **0.9339** и средней абсолютной ошибкой (MAE) **7.28** мин.
> * **Физическая консистентность:** В ходе аппроксимации синтетических данных алгоритм успешно восстановил скрытые термодинамические параметры системы (эффективная энергия активации $E_a/R$ = **-3735.71** и фактор соударений $\ln(A)$ = **-8.20**), доказав корректность генератора данных.
> * **Интерпретируемость:** Анализ `feature_importances_` подтвердил, что ключевым драйвером кинетики в модели выступает концентрация катализатора и температура, что полностью согласуется с реальной химией полиуретанов.
* <p align="center">
